  ...
  function search(sort) {
      var spots = firebase.database().ref("route10/" + sort);
  ...
  function markers(lat1, long1, word, key, sort) {
      //marker
      storageRef.child('images/route10/' + sort + '/' + key + ".png").getDownloadURL().then(function(url) {
          switch (sort) {
              case "spot":
  ...
  function calculateAndDisplayRoute(directionsService, directionsDisplay) {
      var waypts = [];
      var checkboxArray = [{ lat:24.9963802, lng: 121.4363481 }, { lat: 24.9801018, lng: 121.4353283}];
      ...
      directionsService.route({
          origin: { lat:25.0365238, lng:121.4676903},
          destination: { lat:24.9510128, lng: 121.4000982 },
          waypoints: waypts,
