  ...
  function search(sort) {
      var spots = firebase.database().ref("route3/" + sort);
  ...
  function markers(lat1, long1, word, key, sort) {
      //marker
      storageRef.child('images/route3/' + sort + '/' + key + ".png").getDownloadURL().then(function(url) {
          switch (sort) {
              case "spot":
  ...
  function calculateAndDisplayRoute(directionsService, directionsDisplay) {
      var waypts = [];
      var checkboxArray = [{ lat:24.4863439, lng: 120.7839833}, { lat: 24.4743728, lng: 120.7784786}];
      ...
      directionsService.route({
          origin: { lat: 24.4863439, lng:120.7839833},
          destination: { lat: 24.4627339, lng: 120.7497014 },
          waypoints: waypts,
