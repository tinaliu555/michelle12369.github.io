     ...
     $("#send").on("click", function(e) {
        ...
        $("footer").css("background-color", "transparent");
         ...
         switch (select) {
             case 0:
                 var data = f2.database().ref("route1/spot").push({ "lat": nowPos.lat, "long": nowPos.lng, "word": input });
                 uploadImg(picFile, "spot", data.key);

                 break;
             case 1:
                 var data = f2.database().ref("route1/food").push({ "lat": nowPos.lat, "long": nowPos.lng, "word": input });
                 uploadImg(picFile, "food", data.key);
                 break;
             case 2:
                 var data = f2.database().ref("route1/rest").push({ "lat": nowPos.lat, "long": nowPos.lng, "word": input });
                 uploadImg(picFile, "rest", data.key);
                 break;
             case 3:
                 var data = f2.database().ref("route1/repair").push({ "lat": nowPos.lat, "long": nowPos.lng, "word": input });
                 uploadImg(picFile, "repair", data.key);
                 break;
             case 4:
                 var data = f2.database().ref("route1/barrier").push({ "lat": nowPos.lat, "long": nowPos.lng, "word": input });
     ...
     function uploadImg(picFile, sort, key) {
         var uploadTask = storageRef.child('images/' + sort + "/" + key + ".png").put(picFile);
