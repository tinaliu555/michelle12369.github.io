  ...
  function search(sort) {
      var spots = firebase.database().ref("route4/" + sort);
  ...
  function markers(lat1, long1, word, key, sort) {
      //marker
      storageRef.child('images/route4/' + sort + '/' + key + ".png").getDownloadURL().then(function(url) {
          switch (sort) {
              case "spot":
  ...
  function calculateAndDisplayRoute(directionsService, directionsDisplay) {
      var waypts = [];
      var checkboxArray = [ { lat: 24.9763322, lng:121.5850565}, { lat: 24.9679837, lng:121.5850312}];
      ...
      directionsService.route({
          origin: { lat: 24.9875208, lng:121.5738853},
          destination: { lat:24.9875208, lng: 121.5738853},
          waypoints: waypts,
